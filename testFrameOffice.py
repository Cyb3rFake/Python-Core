...
# stats = {'yandex': 120, 'vk': 115, 'google': 99, 'email': 42, 'ok': 98, 'zxczx':-99}
# print(max(stats))
# # print(list(stats.keys()))
# # print(list(stats.values()))
# # print(max(stats.values()))
# # print([list(stats.values()).index(max(stats.values()))]) #получить индекс макстимального ключа
# # print(list(stats.keys())[list(stats.values()).index(max(stats.values()))]) #получить ключ макслимального значения
#
...
#
# import requests
# print(requests.get('https://api.coingate.com/v2/rates/merchant/USD/RUB').text)
...
target = 6
lst = []
if nums.__len__()==2 and sum(nums)==target:
    print('[0,1]')
